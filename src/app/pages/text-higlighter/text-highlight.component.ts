...
export class TextHighlightComponent implements AfterViewInit {
  ...
  private handleTouchStart(event: TouchEvent): void {
    if (event.touches.length === 1) {
      ...
      const { node, offset } = this.getNodeAndOffsetAtPoint(
        touch.clientX,
        touch.clientY
      );
      ...
      if (
        node &&
        this.el.nativeElement.querySelector(".text-container")?.contains(node)
      ) {
        this.startNode = node;
  ...
  private handleTouchMove(event: TouchEvent): void {
    if (this.startNode && event.touches.length === 1) {
      ...
      if (
        ...
        Date.now() - this.touchStartTime > this.selectionStartDelay
      ) {
        this.isSelectingWithTouch = true;

        const { node: endNode, offset: endOffset } =
          this.getNodeAndOffsetAtPoint(touch.clientX, touch.clientY);
        ...
        if (
          ...
          if (
            startPos === Node.DOCUMENT_POSITION_FOLLOWING ||
            ...
            startPos === Node.DOCUMENT_POSITION_PRECEDING ||
            (startPos === Node.DOCUMENT_POSITION_CONTAINS &&
              this.startOffset >= endOffset)
          ) {
      ...
      if (range) {
        ...
        if (startComponent && !selection.isCollapsed) {
          ...
          if (textContainerElement) {
            ...
            const rangeStartPath = this.getNodePath(
              range.startContainer,
              textContainerElement
            );
            const rangeEndPath = this.getNodePath(
              range.endContainer,
              textContainerElement
            );
